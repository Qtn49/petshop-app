...
import { useDayTasksForMonth } from '@/hooks/use-day-tasks';
import { useInvoices } from '@/hooks/use-invoices';
import { useSuppliers } from '@/hooks/use-suppliers';
import { useTenantHref } from '@/hooks/useTenantHref';
import { countDayTasksByDate, dayTasksVisibleOnDate } from '@/lib/day-tasks-calendar';
...
export default function DashboardPage() {
  ...
  const { data: monthTasks = [] } = useDayTasksForMonth(userId, calendarMonth);
  const { data: todayMonthTasks = [] } = useDayTasksForMonth(userId, new Date());
  ...
  const dayTaskCounts = useMemo(() => {
    ...
    return countDayTasksByDate(monthTasks, start, end);
  }, [monthTasks, calendarMonth]);
  ...
  const tasksDueToday = useMemo(() => {
    ...
    return dayTasksVisibleOnDate(todayMonthTasks, key).filter((t) => !t.completed).length;
  }, [todayMonthTasks]);
  ...
  return (
    <div className="flex flex-col gap-4 overflow-hidden pb-2 lg:pb-0">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-3 overflow-hidden auto-rows-fr">
        <WidgetCard className="lg:col-span-2 min-h-[260px] lg:min-h-[300px] overflow-auto">
          {tasksLoading ? (
            <WidgetSkeleton />
          ) : (
            <DashboardCalendar
        ...
        <WidgetCard className="lg:col-span-1 min-h-[200px] lg:min-h-0 overflow-auto">
          <div className="p-3 h-full flex flex-col">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-stone-800 text-sm">Quick actions</h2>
              <Link
                href={tenantHref('/communications')}
        ...
        <WidgetCard className="lg:col-span-1 min-h-0 overflow-auto">
          {linksLoading ? <WidgetSkeleton /> : <SupplierLinks links={links} userId={userId} />}
        </WidgetCard>

        <WidgetCard className="lg:col-span-2 min-h-[320px] lg:min-h-[300px] overflow-hidden">
          {tasksLoading ? (
            <WidgetSkeleton />
          ) : (
            <TodoList
        ...
        <WidgetCard className="lg:col-span-2 min-h-0 overflow-auto">
