...
export default function InvoiceDetailPage() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      router.push(tenantHref(`/invoices/${id}/confirm`));
    } catch {
  ...
  if (!invoice) {
    return (
      <div className="text-center py-12">
        ...
        <Button className="mt-4" onClick={() => router.push(tenantHref('/invoices'))}>
          Back to Invoices
        </Button>
      </div>
    );
      ...
      {parsing ? (
        ...
        <Card title="Parsed Items">
          ...
          <div className="mt-6 flex gap-4">
            <Button variant="secondary" onClick={() => router.push(tenantHref('/invoices'))}>
              Back to Invoices
            </Button>
